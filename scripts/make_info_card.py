...
ROWS = [
    ("host",),
    ("kv", "Now", "AI / ML Engineer-in-training"),
    ("kv", "Focus", "RAG systems · Agentic pipelines"),
    ("kv", "Also", "Cloud & DevOps (K8s, Docker)"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, TypeScript, JavaScript, C#"),
    ("kv", "AI / ML", "RAG, FAISS, BM25, LLM agents"),
    ("kv", "Cloud", "Kubernetes, Docker, Node.js"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "ragfin — hybrid RAG for financial analysis, Recall@10=0.806"),
    ("bul", "concord — cross-dept governance agent w/ human-in-the-loop"),
    ("bul", "weather-k8s — real-time dashboard on Kubernetes w/ HPA"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">ruthik@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">ruthik</tspan><tspan fill="{MUTED}">@</tspan>'
